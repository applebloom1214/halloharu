# 할로하루 HalloHaru

할로하루는 사용자가 하루를 가볍게 기록하고, 부담 없는 공감과 웃음을 주고받을 수 있는 하루 기록 서비스입니다.

## 프로젝트 목적

대부분의 SNS는 뛰어난 외모, 능력, 인맥, 화려한 일상처럼 보여줄 것이 많은 사람에게 유리합니다.

반면 평범한 일상을 보내는 사람들은 글을 올려도 충분한 반응을 받기 어렵고, 무플과 무관심이 반복되면 점점 기록의 흥미를 잃게 됩니다.

할로하루는 평범한 하루도 부담 없이 남기고, 가벼운 리액션으로 공감받을 수 있는 공간을 목표로 합니다.

## MVP 기능

- 회원가입 / 로그인
- 가벼운 하루 기록 작성
- 하루 기록 목록 보기
- 이모지 / 이모티콘 공감 리액션
- 내가 쓴 기록 보기
- 유머 공간 Coming soon 페이지

## 기술 스택

### 현재 사용 중

- Next.js
- TypeScript
- Tailwind CSS
- Supabase
- PostgreSQL
- Git / GitHub

### 도입 예정

- Vercel

## 개발 진행 상황

### 완료

* [x] 서비스명 결정
* [x] 로고 / 파비콘 방향 결정
* [x] MVP 기획서 초안 작성
* [x] 화면 목록 정리
* [x] GitHub 저장소 생성
* [x] Next.js 프로젝트 생성
* [x] 첫 화면 구현
* [x] 하루 기록 입력 상태 관리
* [x] 빈 기록 등록 방지
* [x] 300자 제한 및 글자 수 표시
* [x] 하루 기록 목록 구현
* [x] 작성 날짜 표시
* [x] 화면을 컴포넌트 단위로 분리
* [x] 게시글 수정 및 취소 기능 구현
* [x] 게시글 삭제 기능 구현
* [x] 공감, 응원, 미소 리액션 기능 구현
* [x] Supabase 프로젝트 생성 및 연결
* [x] posts 테이블 생성
* [x] Supabase 게시글 조회 기능 구현
* [x] Supabase 게시글 작성 기능 구현
* [x] Supabase 게시글 수정 기능 구현
* [x] Supabase 게시글 삭제 기능 구현
* [x] Supabase 게시글 리액션 수 저장 기능 구현
* [x] 기능별 Git 커밋 및 GitHub push


### 다음 개발 예정

* [ ] Supabase Auth 설정
* [ ] 회원가입 / 로그인 / 로그아웃 구현
* [ ] 게시글에 작성자 정보 연결
* [ ] 작성자만 자신의 기록을 수정, 삭제 하도록 설정
* [ ] 사용자별 리액션 저장
* [ ] 내가 쓴 기록 보기
* [ ] 유머 공간 Coming soon 페이지 구현
* [ ] Vercel 배포
